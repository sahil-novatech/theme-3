...
export default function Blogs() {
  ...
  useEffect(() => {
    if(blogs?.data?.data?.blogs?.data){
      // setHasMorePage(!!blogs.data.data.blogs.next_page_url)
      setTotal(blogs.data.data.total);
      setBlogsData((data: BlogData[]) => blogs.data.data.blogs.current_page === 1 ? blogs.data.data.blogs.data : ([...data, ...blogs.data.data.blogs.data]))
    }
  }, [blogs?.data?.data])
    ...
    <div className="w-full mt-6">
      {blogsData.length > 0 && <div className="flex items-start justify-between wow animate__animated animate__fadeInUp">
        <div>
          {blogs?.isLoading ? <></>
            : <>
              <p className="font-bold text-[20px]">{total ? `Total ${total} Blogs Found` : "No Blogs Found"}</p>
            </>
          }
        </div>
      </div>}
